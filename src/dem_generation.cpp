...
void DEM::setCameraParameters(int width, int height, float cx, float cy, float fx, float fy)
{
    ...
    transform(xArray_const.begin(),
              xArray_const.end(),
              xArray_const.begin(),
              bind1st(std::plus<float>(), -cx));
    transform(yArray_const.begin(),
              yArray_const.end(),
              yArray_const.begin(),
...
void DEM::setPcFiltersParameters(Eigen::Vector4f filter_box_min,
...
std::string DEM::constructProductPath(std::string identifier,
                                      ...
                                      std::string left_right)
{
    ...
    return default_save_location + camera_name + "_" + left_right + identifier + "_"
           + sensor_data_time + file_ending;
}
...
void DEM::distance2pointCloud(std::vector<float> distance)
{
    ...
    std::transform(xArray_const.begin(),
                   xArray_const.end(),
                   distance.begin(),
                   xArray.begin(),
                   std::multiplies<float>());

    std::transform(yArray_const.begin(),
                   yArray_const.end(),
                   distance.begin(),
                   yArray.begin(),
                   std::multiplies<float>());
...
void DEM::setPointCloud(pcl::PointCloud<pcl::PointXYZ>& input_cloud)
{
    ...
    cloud_input_p->resize(input_cloud.size());  // when saving unfiltered, handles the possibility
                                                // the size of the pointclous is changing

    Eigen::Matrix3d m;
    // pointcloud of tof needs to be flipped 180 around Z axis.
    // At the moment this is a custom solution
...
void DEM::compressProducts(bool enable_compression, int compression_level)
{
    ...
    this->compression_level = min(max(compression_level, 0), 100);
}
...
int DEM::pointCloud2Mesh(bool use_filtered)
{
    ...
    {
        LOG_WARN_S
            << "You asked for a filtered PC to be put in a mesh but you forgot to call the filter!";
    }
...
void DEM::mapTexture2MeshUVnew(pcl::TextureMesh& tex_mesh,
                               pcl::TexMaterial& tex_material,
                               std::vector<std::string>& tex_files)
{
...
std::string DEM::getMeshPath() { return mesh_location; }
std::string DEM::getPointCloudPath() { return point_cloud_obj_location; }
std::string DEM::getImageLeftPath() { return color_frame_location_left; }
std::string DEM::getImageRightPath() { return color_frame_location_right; }
std::string DEM::getDistanceFramePath() { return distance_frame_location; }